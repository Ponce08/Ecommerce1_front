  ...
  return (
    <>
      ...
      <div className="bg-colorBackground relative mt-[100px] md:pb-32 lg:pb-[225px]">
        ...
        {userLogin && (favorites[userLogin.id] || []).length === 0 && (
        ...
        <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 sm:py-24 sm:py-8 lg:max-w-7xl lg:px-8 lg:py-8">
          ...
          >
            {userLogin &&
              (favorites[userLogin.id] || []).map((product: FavoriteCard) => (
                <div key={product.id}>
                      ...
                      <div>
                        <HeartIcon
                          className={`mr-4 h-6 w-6 text-purple-700 mt-2 ml-2 ${
                            (favorites[userLogin.id] || []).some((fav) => fav.id === product.id) ? 'fill-purple-700' : ''
                          } focus:outline-none animate-scale`}
                          id={`favorite${product.id}`}
                          data-tooltip-content={
                            (favorites[userLogin.id] || []).some((fav) => fav.id === product.id)
